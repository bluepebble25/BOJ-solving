...
/*
  ...
  - 토마토 뭉텅이 돌며 다음에 탐색할 토마토들을 새 배열에 담아놓자. 근데 중복될 수 있으므로 Set 사용
*/
...
function solution() {
  ...
  while (true) {
    const prev = Array.from(prevTomatoes);
    prevTomatoes.clear();
    ...
    prev.forEach((dxy) => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        if (nx >= 0 && nx < M && ny >= 0 && ny < N && box[ny][nx] === 0) {
          box[ny][nx] = 1;
          ripeCount++;
          prevTomatoes.add([nx, ny]);
        }
      }
    });
    if (prevTomatoes.size === 0) break;
    day++;
  }
